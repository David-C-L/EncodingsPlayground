...
inline std::vector<DatasetEntry<int64_t>> int64Datasets() {
    ...
    // Pre/post/level tree-position ids from an XMark XML corpus, bit-packed into one
    // int64 key (level:8 | pre:28 | post:28 — see Datasets/XMark/README.md). Two node
    // models: "Elements" numbers element nodes only; "Full" additionally numbers
    // attributes and text nodes in the same pre/post space (Grust's XPath Accelerator
    // "database node" model). Generated by Datasets/XMark/generate_prepost_ids.py.
    const std::filesystem::path xmarkElements = root / "XMark" / "prepost_ids_elements.parquet";
    detail::pushIfPresent<Elem>(
        d, {"XMarkPrePostElements",
            std::make_shared<DeferredParquetColumn<Elem>>(xmarkElements, "prepost_id"),
            true, xmarkElements});

    const std::filesystem::path xmarkFull = root / "XMark" / "prepost_ids_full.parquet";
    detail::pushIfPresent<Elem>(
        d, {"XMarkPrePostFull",
            std::make_shared<DeferredParquetColumn<Elem>>(xmarkFull, "prepost_id"),
            true, xmarkFull});
